...
# An outlier is a data point that falls outside of 1.5 times of an interquartile range above the 3rd quartile and below the 1st quartile.

# In[8]:


# Code from https://careerfoundry.com/en/blog/data-analytics/how-to-find-outliers/
...
ans4_1 = find_outliers_IQR(df.price) 

print('Q4.1: the list of all outliers is as follows:\n', ans4_1)
...
#4.2 How many outliers are there?
ans4_2 = find_outliers_IQR(df.price).size
print('Q4.2: The number of outliers is', ans4_2)
...
print('Q4.3: The shape of the distribution of the car prices is right-skewed as it has a tail on the right side') 
...
ans6_1 = find_outliers_IQR(df.log_price) 
...
df[num_col.tolist()[:-2]] = scaler.fit_transform(df[num_col.tolist()[:-2]])
...
hm = sns.heatmap(df[num_col.tolist()].corr(), annot = False)
...
df[num_col.tolist()].corr().log_price.abs().sort_values(ascending=False)[0:11]
...
df[num_col.tolist()].corr().log_price[df[num_col.tolist()].corr().log_price>0.5].sort_values(ascending=False)
...
df[num_col.tolist()].corr().log_price[df[num_col.tolist()].corr().log_price<-0.5].sort_values()
...
#df[num_col.tolist()].corr().log_price[df[num_col.tolist()].corr().log_price>0.5].index


# In[26]:


selected_var = df[num_col.tolist()].corr().log_price[df[num_col.tolist()].corr().log_price>0.5].index 
df[selected_var]
sns.pairplot(df[selected_var])
...
print('Q12: There are clear linear relationships between log_price and the variables with significant correlation')
...
ans13 = df[num_col.tolist()].corr().log_price[df[num_col.tolist()].corr().log_price.abs()>0.5].sort_values(ascending=False)[[2]] 

print('Q13:the variable with the highest correlation to the output log_price:\n', ans13)
...
id1=888
np.random.seed(id1)
df_train, df_test = train_test_split(df, train_size = 0.7, test_size = 0.3, random_state = id1)
...
model1 = smf.ols(formula='log_price ~ curbweight', data=df_train).fit()
...
X_train = df_train['curbweight']
y_train = df_train['log_price']

df_train['model1_prediction'] = model1.predict(X_train)
df_train['model1_residual'] = y_train - model1.predict(X_train)

print('Q16: the coefficient of determination (R-squared):', r2_score(y_train.values, df_train['model1_prediction'].values))
...
ans17 = model1.params.Intercept

print('Q17: the intercept of the model:', ans17)
...
#model1.params.curbweight

ans18 = model1.params.curbweight

print('Q18: the intercept of the model:', ans18)
...
df_test.head()
df_test.loc[:,['curbweight','log_price', 'model1_prediction']].head()
...
print('Q20.1: Mean Absolute Error:', mean_absolute_error(y_test.values , df_test['model1_prediction'].values))
print('Q20.2: Mean Squared Error:', mean_squared_error(y_test.values , df_test['model1_prediction'].values))
#print('Root Mean Squared Error:', np.sqrt(mean_squared_error(y_test.values , df_test['model1_prediction'].values)))
print('Q20.3: R-squared:',r2_score(y_test.values , df_test['model1_prediction'].values))
...
(
    ggplot(df_train) + aes(x = 'curbweight', y='log_price')
